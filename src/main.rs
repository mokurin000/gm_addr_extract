...
fn main() -> Result<()> {
    let lib_data = fs::read("data/com.PigeonGames.Phigros/lib/libil2cpp.so")?;
    let lib = ElfBytes::<LittleEndian>::minimal_parse(&lib_data)?;
    ...
    let text_section_bytes = &lib_data[text_addr..text_addr_end];
